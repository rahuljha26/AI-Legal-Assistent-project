...
One interaction to know: when a send carries [`recipientVariables`](/docs/reference/message#recipient-variables) and the adapter has no native batch support, each recipient becomes its own send and the key gets a per-recipient suffix (`<key>:<address>`). Retries then dedupe per recipient instead of dropping the whole batch, and the longer key still counts toward provider key-length limits.
